...
describe('AuthConsumer', () => {
  ...
  beforeEach(async () => {
    ...
    consumer = module.get<AuthConsumer>(AuthConsumer);
  ...
  describe('handleMessage', () => {
    ...
    it('should ignore duplicate message if key exists in Redis', async () => {
      ...
      expect(mockAuthRepository.deleteAllUserTokens).not.toHaveBeenCalled();
    });

    it('should process UserBanned event and delete all user tokens', async () => {
      const messagePayload = {
        messageId: 'msg-ban-1',
        eventType: RabbitMQEvent.UserBanned,
        payload: { userId: 'u-banned-1' },
      };
    ...
    it('should re-queue to retry queue on processing error (attempt 1)', async () => {
      ...
      mockRedis.get.mockResolvedValue(null);
      mockAuthRepository.deleteAllUserTokens.mockRejectedValue(
