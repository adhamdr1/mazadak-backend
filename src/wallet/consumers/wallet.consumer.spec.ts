import { Test, TestingModule } from '@nestjs/testing';
import { WalletConsumer } from './wallet.consumer';
import { ConfigService } from '@nestjs/config';
import { getConnectionToken } from '@nestjs/mongoose';
import { getRedisConnectionToken } from '@nestjs-modules/ioredis';
import { OutboxService } from '../../infrastructure/outbox/outbox.service';
import { TransactionService } from '../../transaction/transaction.service';
import {
  WALLET_RETRY_QUEUE_5S,
  X_RETRY_COUNT,
} from '../../infrastructure/rabbitmq/rabbitmq.constants';
import { RabbitMQEvent } from '../../infrastructure/rabbitmq/rabbitmq-event.types';
import * as amqpManager from 'amqp-connection-manager';
import type * as amqplib from 'amqplib';
import type { ClientSession } from 'mongoose';

jest.mock('amqp-connection-manager', () => ({
  connect: jest.fn(),
}));

const mockWalletRepository = {
  creditBalance: jest.fn(),
};

const mockOutboxService = {
  saveEvent: jest.fn().mockResolvedValue(undefined),
};

const mockTransactionService = {
  findByIdWithinSession: jest.fn(),
  markWalletCredited: jest.fn().mockResolvedValue(undefined),
};

const mockConfigService = {
  getOrThrow: jest.fn().mockReturnValue('amqp://localhost:5672'),
};

const mockRedis = {
  get: jest.fn(),
  set: jest.fn().mockResolvedValue('OK'),
};

const mockSession: Partial<ClientSession> = {
  startTransaction: jest.fn(),
  commitTransaction: jest.fn().mockResolvedValue(undefined),
  abortTransaction: jest.fn().mockResolvedValue(undefined),
  endSession: jest.fn().mockResolvedValue(undefined),
};

const mockMongooseConnection = {
  startSession: jest.fn().mockResolvedValue(mockSession),
};

type HandleMessageFn = (
  msg: amqplib.ConsumeMessage,
  channel: amqplib.Channel,
) => Promise<void>;

describe('WalletConsumer', () => {
  let consumer: WalletConsumer;
  let handleMessage: HandleMessageFn;
  let mockChannel: {
    prefetch: jest.Mock;
    consume: jest.Mock;
    ack: jest.Mock;
    reject: jest.Mock;
    sendToQueue: jest.Mock;
  };
  let mockChannelWrapper: {
    close: jest.Mock;
  };
  let mockConnection: {
    on: jest.Mock;
    createChannel: jest.Mock;
    close: jest.Mock;
  };

  beforeEach(async () => {
    mockChannel = {
      prefetch: jest.fn().mockResolvedValue(undefined),
      consume: jest.fn().mockResolvedValue(undefined),
      ack: jest.fn(),
      reject: jest.fn(),
      sendToQueue: jest.fn(),
    };

    mockChannelWrapper = {
      close: jest.fn().mockResolvedValue(undefined),
    };

    mockConnection = {
      on: jest.fn(),
      createChannel: jest.fn().mockReturnValue(mockChannelWrapper),
      close: jest.fn().mockResolvedValue(undefined),
    };

    (amqpManager.connect as jest.Mock).mockReturnValue(mockConnection);

    const module: TestingModule = await Test.createTestingModule({
      providers: [
        WalletConsumer,
        {
          provide: 'IWalletRepository',
          useValue: mockWalletRepository,
        },
        {
          provide: OutboxService,
          useValue: mockOutboxService,
        },
        {
          provide: TransactionService,
          useValue: mockTransactionService,
        },
        {
          provide: ConfigService,
          useValue: mockConfigService,
        },
        {
          provide: getRedisConnectionToken('default'),
          useValue: mockRedis,
        },
        {
          provide: getConnectionToken(),
          useValue: mockMongooseConnection,
        },
      ],
    }).compile();

    consumer = module.get<WalletConsumer>(WalletConsumer);
    consumer.onApplicationBootstrap();

    const consumerWithHandleMessage = consumer as unknown as {
      handleMessage: HandleMessageFn;
    };
    handleMessage = (msg: amqplib.ConsumeMessage, ch: amqplib.Channel) =>
      consumerWithHandleMessage.handleMessage(msg, ch);
  });

  afterEach(() => {
    jest.clearAllMocks();
  });

  it('should be defined', () => {
    expect(consumer).toBeDefined();
  });

  describe('lifecycle', () => {
    it('should connect and create channel on application bootstrap', () => {
      expect(amqpManager.connect).toHaveBeenCalledWith([
        'amqp://localhost:5672',
      ]);
      expect(mockConnection.createChannel).toHaveBeenCalled();
    });

    it('should close connection and channel on module destroy', async () => {
      await consumer.onModuleDestroy();

      expect(mockChannelWrapper.close).toHaveBeenCalled();
      expect(mockConnection.close).toHaveBeenCalled();
    });
  });

  describe('handleMessage', () => {
    it('should reject invalid JSON to DLQ immediately', async () => {
      const msg = {
        content: Buffer.from('invalid-json'),
      } as unknown as amqplib.ConsumeMessage;

      await handleMessage(msg, mockChannel as unknown as amqplib.Channel);

      expect(mockChannel.reject).toHaveBeenCalledWith(msg, false);
    });

    it('should ignore duplicate message if key exists in Redis', async () => {
      const messagePayload = {
        messageId: 'msg-123',
        eventType: RabbitMQEvent.WalletDepositInitiated,
        payload: { walletId: 'w1', amount: 500, transactionId: 'tx1' },
      };

      const msg = {
        content: Buffer.from(JSON.stringify(messagePayload)),
        properties: { headers: {} },
      } as unknown as amqplib.ConsumeMessage;

      mockRedis.get.mockResolvedValue('1');

      await handleMessage(msg, mockChannel as unknown as amqplib.Channel);

      expect(mockChannel.ack).toHaveBeenCalledWith(msg);
      expect(mockWalletRepository.creditBalance).not.toHaveBeenCalled();
    });

    it('should skip processing if transaction not found', async () => {
      const messagePayload = {
        messageId: 'msg-456',
        eventType: RabbitMQEvent.WalletDepositInitiated,
        payload: { walletId: 'w1', amount: 500, transactionId: 'tx1' },
      };

      const msg = {
        content: Buffer.from(JSON.stringify(messagePayload)),
        properties: { headers: {} },
      } as unknown as amqplib.ConsumeMessage;

      mockRedis.get.mockResolvedValue(null);
      mockTransactionService.findByIdWithinSession.mockResolvedValue(null);

      await handleMessage(msg, mockChannel as unknown as amqplib.Channel);

      expect(mockSession.commitTransaction).toHaveBeenCalled();
      expect(mockWalletRepository.creditBalance).not.toHaveBeenCalled();
      expect(mockChannel.ack).toHaveBeenCalledWith(msg);
    });

    it('should skip processing if transaction already walletCredited', async () => {
      const messagePayload = {
        messageId: 'msg-456',
        eventType: RabbitMQEvent.WalletDepositInitiated,
        payload: { walletId: 'w1', amount: 500, transactionId: 'tx1' },
      };

      const msg = {
        content: Buffer.from(JSON.stringify(messagePayload)),
        properties: { headers: {} },
      } as unknown as amqplib.ConsumeMessage;

      mockRedis.get.mockResolvedValue(null);
      mockTransactionService.findByIdWithinSession.mockResolvedValue({
        _id: 'tx1',
        walletCredited: true,
      });

      await handleMessage(msg, mockChannel as unknown as amqplib.Channel);

      expect(mockSession.commitTransaction).toHaveBeenCalled();
      expect(mockWalletRepository.creditBalance).not.toHaveBeenCalled();
      expect(mockChannel.ack).toHaveBeenCalledWith(msg);
    });

    it('should credit wallet balance and save outbox event on success', async () => {
      const messagePayload = {
        messageId: 'msg-789',
        eventType: RabbitMQEvent.WalletDepositInitiated,
        payload: { walletId: 'w1', amount: 500, transactionId: 'tx1' },
      };

      const msg = {
        content: Buffer.from(JSON.stringify(messagePayload)),
        properties: { headers: {} },
      } as unknown as amqplib.ConsumeMessage;

      mockRedis.get.mockResolvedValue(null);
      mockTransactionService.findByIdWithinSession.mockResolvedValue({
        _id: 'tx1',
        walletCredited: false,
      });

      mockWalletRepository.creditBalance.mockResolvedValue({
        _id: 'w1',
        userId: 'u1',
        balance: 1500,
      });

      await handleMessage(msg, mockChannel as unknown as amqplib.Channel);

      expect(mockWalletRepository.creditBalance).toHaveBeenCalledWith(
        'w1',
        500,
        mockSession,
      );
      expect(mockTransactionService.markWalletCredited).toHaveBeenCalledWith(
        'tx1',
        mockSession,
      );
      expect(mockOutboxService.saveEvent).toHaveBeenCalledWith(
        RabbitMQEvent.WalletDeposited,
        {
          userId: 'u1',
          amount: 500,
          transactionId: 'tx1',
        },
        mockSession,
        'tx1',
      );
      expect(mockSession.commitTransaction).toHaveBeenCalled();
      expect(mockRedis.set).toHaveBeenCalled();
      expect(mockChannel.ack).toHaveBeenCalledWith(msg);
    });

    it('should re-queue to retry queue on processing error (attempt 1)', async () => {
      const messagePayload = {
        messageId: 'msg-err-1',
        eventType: RabbitMQEvent.WalletDepositInitiated,
        payload: { walletId: 'w1', amount: 500, transactionId: 'tx1' },
      };

      const msg = {
        content: Buffer.from(JSON.stringify(messagePayload)),
        properties: { headers: {} },
      } as unknown as amqplib.ConsumeMessage;

      mockRedis.get.mockResolvedValue(null);
      mockTransactionService.findByIdWithinSession.mockRejectedValue(
        new Error('DB Lock Timeout'),
      );

      await handleMessage(msg, mockChannel as unknown as amqplib.Channel);

      expect(mockSession.abortTransaction).toHaveBeenCalled();
      expect(mockChannel.sendToQueue).toHaveBeenCalledWith(
        WALLET_RETRY_QUEUE_5S,
        msg.content,
        expect.objectContaining({
          headers: {
            [X_RETRY_COUNT]: 1,
          },
        }),
      );
      expect(mockChannel.ack).toHaveBeenCalledWith(msg);
    });

    it('should reject to DLQ if max retries exceeded', async () => {
      const messagePayload = {
        messageId: 'msg-err-max',
        eventType: RabbitMQEvent.WalletDepositInitiated,
        payload: { walletId: 'w1', amount: 500, transactionId: 'tx1' },
      };

      const msg = {
        content: Buffer.from(JSON.stringify(messagePayload)),
        properties: { headers: { [X_RETRY_COUNT]: 3 } },
      } as unknown as amqplib.ConsumeMessage;

      mockRedis.get.mockResolvedValue(null);
      mockTransactionService.findByIdWithinSession.mockRejectedValue(
        new Error('Fatal DB Failure'),
      );

      await handleMessage(msg, mockChannel as unknown as amqplib.Channel);

      expect(mockSession.abortTransaction).toHaveBeenCalled();
      expect(mockChannel.reject).toHaveBeenCalledWith(msg, false);
    });
  });
});
